...
struct Preset: Codable, Equatable {
    ...
    var audioThickness: Double = 0.012
    var audioSpan: Double = 0.8
    var audioMirror: Bool = true
    var audioGlow: Double = 0.02
    var audioFill: Double = 0
    var audioCenterY: Double = 0.5
    var audioRadius: Double = 0.5
    var audioBars: Double = 48
    var audioReact: Double = 0.5
    var audioSmoothing: Double = 0.5
    ...
    var chatStyle: UInt32 = 0
    var chatTypeSpeed: Double = 22
    var chatTerminalY: Double = 0.72
    var chatCursorWidth: Double = 0.5
    var chatCursorBlink: Double = 1.6
    ...
    init(from decoder: Decoder) throws {
        ...
        audioThickness = c.value(.audioThickness, d.audioThickness)
        audioSpan = c.value(.audioSpan, d.audioSpan)
        audioMirror = c.value(.audioMirror, d.audioMirror)
        audioGlow = c.value(.audioGlow, d.audioGlow)
        audioFill = c.value(.audioFill, d.audioFill)
        audioCenterY = c.value(.audioCenterY, d.audioCenterY)
        audioRadius = c.value(.audioRadius, d.audioRadius)
        audioBars = c.value(.audioBars, d.audioBars)
        audioReact = c.value(.audioReact, d.audioReact)
        audioSmoothing = c.value(.audioSmoothing, d.audioSmoothing)
        audioGain = c.value(.audioGain, d.audioGain)
        ...
        chatStyle = c.value(.chatStyle, d.chatStyle)
        chatTypeSpeed = c.value(.chatTypeSpeed, d.chatTypeSpeed)
        chatTerminalY = c.value(.chatTerminalY, d.chatTerminalY)
        chatCursorWidth = c.value(.chatCursorWidth, d.chatCursorWidth)
        chatCursorBlink = c.value(.chatCursorBlink, d.chatCursorBlink)
